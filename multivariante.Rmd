...
##### Observaciones
La Gráfica muestra *Carreras Impulsadas (RBI)* contra los *Veces al bate*. El color está representado por las posiciones del terreno *(POS)* y tamaño por veces al bate *(AB)*. 
```{r echo=FALSE, results='asis', message=FALSE}
# print(p_impulseByPos)
ggplotly(p_impulseByPos, tooltip = c("text","label","x","y"))
```
Homogeneidad entre las posiciones. Relación lineal entre las variables con cierta dispersión.
...
#### Relación de Home Runs y Veces al Bate {.tabset}
Si bateador hace contacto con la pelota de una manera que le permita recorrer todas las bases y anotar una carrera se ha realizado un home run o cuadrangular. La forma más común es cuando el bateador golpea la pelota y la saca al exterior del campo de juego. Es uno de los aspectos más populares y difíciles para el bateador, es por ello que los jugadores que usualmente logran hacer este tipo de jugadas son muy valorados en el terreno.
...
#### Relación entre bases robadas y atrapados robando {.tabset}
