...
#### 1. ¿Cómo batean los jugadores por posiciones? {.tabset}

##### Observaciones
Gráfica de *Número de juegos (G)* vs *promedio de bateo (AVG)*. El color está representado por las posiciones del terreno *(POS)* y tamaño por *Average (AVG)*
```{r echo=FALSE, results='asis', message=FALSE}
ggplotly(p_playerByPos, tooltip = c("text", "label", "x" , "y"))
```
Se observa una relación lineal entre las variables  *G* y *AVG* con bastante dispersión. No se aprecia gran diferencia entre las *impulsadas* y las *posiciones*. Las pendientes de las líneas de tendencias son semejantes y bastante bajas. Las posiciones *Right fields (RF)* y *Of Fild (OF)* presentan pendientes muy bajas, implica que no tienen sufieicntes impulsadas por juegos.
...
```{r echo=FALSE, results='asis'}
ggplotly(p_playerByPosNames, tooltip = c("text", "label", "x" , "y"))
```
Los mejores jugadores tendrán mayor *AVG* por *juegos (G)*: Los mejores jugadores con un *AVG* mayor que 350 son Joe Jackson, Rogers Hornsby y Ty Cobb.

---

#### 2. ¿Cómo impulsan por posiciones? {.tabset}
...
#### 3. Relación de Home Runs y Veces al Bate {.tabset}
Si el bateador hace contacto con la pelota de manera que le permita recorrer tdas las bases y anotar una carrera, se ha realizado un home run o cuadrangular. La forma más común es cuando el bateador golpea la pelota y la saca al exterior del campo de juego. Es uno de los jugadas más populares y difíciles para el bateador, por ello los jugadores que usualmente logran hacer este tipo de jugadas son muy valorados en el terreno.
...
```
Mucha dispersión y aglomeración de datos por debajo de *400 HR*, esto se debe precisamente a lo difícil de anotar home runs. Las líneas de tendencia marcan los *HR* por posiciones: *los Tercera Base (3B)*, * Cathcers "C"*, *los Of Field OF* y *Right field RF* presentan una pendiente ligeramente mayor que el resto, por tanto en general son mejores anotando home runs. 
...
```
La gráfica muestra que la mayoría de los jugadores no roban demasiadas bases. Según las líneas de tendencia que indican las posiciones, los jugadores q más roban son el *Segunda Base (2B)*, *Left Field (LF)* y *Campo Corto (SS)*. 
