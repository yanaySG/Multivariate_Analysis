...
Se utilizan las técnicas de PCA si existen altas correlaciones entre las variables, ya que esto es indicativo de que existe información redundante y, por tanto, pocos factores explicarán gran parte de la variabilidad total.  

La elección de las componentes se realiza de tal forma que la primera recoja la mayor proporción posible de la variabilidad original; la segunda componente recoge la máxima variabilidad posible no recogida por la primera, y así sucesivamente. Del total de factores se elegirán aquéllos que recojan el porcentaje de variabilidad que se considere suficiente. A éstos se les denomina componentes principales del dataset.

#### **Variabilidad de las componenes**

La proporción de variabilidad retenida por los componentes principales se puede extraer mediante valores propios *(eigen values)*. Los datos de la varianza son muy importantes para saber cuántas componentes principales serán utilizados para el análisis. 

**¿Cómo saber cuantas componentes escoger?** Componentes con un valor propio > 1 representan más varianza de la que tiene en cuenta una de las variables originales en los datos estandarizados. Ésto, junto a la varianza acumulada, son los parámetros utilizados como punto de corte para determinar las componentes a escoger.


```{r echo=FALSE, results='asis'}
kable(importance_of_components, "html", caption = "Importance of components",
